...
def is_possible(input):
    ...
    for i in range(3):
       r = 3 * i
       if input[r] == input[r + 1] == input[r + 2] != "_":
           counter += 1
       if input[i] == input[i + 3] == input[i + 6] != "_":
           counter += 1
    if input[0] == input[4] == input[8] != "_":
        counter += 1
    if input[2] == input[4] == input[6] != "_":
        counter += 1
...
def plain_text(data):
    return "".join([c for r in data for c in r]).replace(" ", "_")
...
if __name__ == "__main__":
    ...
    print_grid(data)
    while get_current_stat(plain_text(data)):
        play_turn(data)